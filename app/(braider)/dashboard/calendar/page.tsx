...
export default async function CalendarPage() {
  ...
  bookings?.forEach((b) => {
    const key = format(startOfDay(new Date(b.scheduled_at)), 'yyyy-MM-dd');
  ...
  return (
    <div>
      <PageHeader
        title="This week"
        description={`${format(weekStart, 'MMM d')} – ${format(addDays(weekStart, 6), 'MMM d')}`}
      />
        ...
        {days.map((day) => {
          ...
          return (
            ...
            >
              ...
              <p className="font-display text-2xl text-ink">{format(day, 'd')}</p>
